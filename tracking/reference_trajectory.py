...
def read_all_refs(ref_dir=REF_DIR):
    ...
    for i, file in enumerate(files):
        res.append(np.loadtxt(os.path.join(ref_dir, file), dtype=int, delimiter=","))
    return res, nbs
...
def add_ref_trajectories(nbs, ref_dir=REF_DIR, tra_dir=TRACK_DIR):
    ...
    for nb in nbs:
        tra = np.loadtxt(os.path.join(tra_dir, "%06d.txt" % nb), dtype=int, delimiter=",")
...
def calculate_overlap(tra_dir=TRACK_DIR, ref_dir=REF_DIR):
    ...
    res = np.zeros((len(nbs), 3), dtype=int)
