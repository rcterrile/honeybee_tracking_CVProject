...
def build_trajectories():
    ...
    t1 = np.zeros((FR2-FR1+1, fr_preds.shape[0], 4+EMB_SIZE), dtype=float)
    last_seen = np.zeros([t1.shape[1], LAST_N], dtype=int)
    lens = [1]*fr_preds.shape[0]
    for i in range(fr_preds.shape[0]):
        t1[0,i,:] = fr_preds[i,:]

    t = np.zeros((t1.shape[0], t1.shape[1] * n_dim, t1.shape[2]), dtype=float)
    t[:, :t1.shape[1], :] = t1
    tra_cols = np.zeros((t1.shape[1] * n_dim), dtype=bool)
    tra_cols[:t1.shape[1]] = True
    lens.extend([-1] * (t1.shape[1]*n_dim))
    last_seen = np.append(last_seen, np.zeros((last_seen.shape[0]*(n_dim-1), LAST_N), dtype=int) - 1, axis=0)
...
def sort_trajectories():
    ...
    files.sort()
    all_tras = []
    tra_lens = np.zeros((len(files), 2), dtype=int)
